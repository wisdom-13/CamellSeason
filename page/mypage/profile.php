<?php if($_SESSION["p3"]!="0"){ move("/index.php/mypage/update"); return; } ?>

<?php $row = fetchAll("SELECT * FROM PLAYER where p_no ='{$_SESSION['p1']}'"); ?>

<form action="/index.php/ok/mypage/add_profile/<?= $_SESSION["no"] ?>" enctype="multipart/form-data" method="post" id="form1" runat="server">
<div class="profile_word">"<input style="width: 80%; text-align: center" type="text" name="word" placeholder="한마디를 입력해주세요." required>"</div>
<div class="profile_body" style="margin: 20px 0">
	<label style="width: 700px">
		<input type="file" style="display: none" class="form-control" onchange="bodyImg(this);" id="img" name="body" required>
		<img style="margin: 10px 0; width: 700px; min-height: 300px; line-height: 300px" id="blah" src="#" alt="이미지를 첨부해주세요. (700x자유)">
	</label>
</div>
	
<div class="profile_content">dd<?php echo "dd".$_SESSION["p1"] ?>
	...
	<div class="profile_mini">
		<div class="mini_head">
			<label style="width: 250px;">
				<input type="file" style="display: none" class="form-control" onchange="headImg(this);" id="img" name="head" required>
				<img style="width: 250px; min-height: 250px; line-height: 250px" id="blah2" src="#" alt="이미지를 첨부해주세요. (300x300)">
			</label>
		</div>
		<div class="mini_text">
			<br>
			<p style="margin-bottom: 10px;">
				...
				성별 : <?= $SEX[$row->p_sex] ?><br>
				생일 : <?= $row->p_brith ?><br>
				신체 : <input type="text" style="color: #fff;" name="hight" placeholder="cm / kg" required> <br>
			</p>
	...
	<div class="profile_box">
		<textarea name="look" placeholder="전신의 유무와 상관없이 최소 3줄 이상 서술해세요." required></textarea>
	</div>
	...
	<div class="profile_box">
		<textarea name="info" placeholder="최소 3줄 이상 서술해주세요. 출처 표기를 한 인용 글, 캐릭터 이입 한마디나 키워드 작성 후 서술도 허용하고 있습니다." required></textarea>
	</div>
	<img style="margin-left: -21.5px; margin-top: -20px;" src="/common/img/profile/view/bottom.png">
	<img style="margin-left: -27px" src="/common/img/profile/view/etc.png">
	<div class="profile_box">
		<textarea name="etc" placeholder="기타 이미지가 있을 경우 <img width='이미지 가로 사이즈px(700px이하)' src='이미지 주소'>로 입력해주시기 바라며, 이미지 첨부에 어려움이 있는 경우 총괄계로 문의해주세요." required></textarea>
	</div>
	<img style="margin-left: -21.5px; margin-top: -20px;" src="/common/img/profile/view/bottom.png">
